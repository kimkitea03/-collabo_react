...
function App({ user }) {
    ...
    //아직 backend에서 읽오 오지 못한 경우를 대비한 코딩입니다.
    if (loading === true) {
        return (
    ...
    const buyNow = async () => {
        ...
        try {
            ...
            // 상세 보기 페이지에서는 무조건 1개의 상품만 주문을 할 수 있습니다.
            const parameters = {
                memberId: user.id,
                status: 'PENDING',
                orderItems: [{
                ...
                <Row className="g-0">
                    ...
                    <Col md={8}>
                        <Card.Body>
                            ...
                            {/* 구매 수량 입력란 */}
                            {/* as={Row}는 렌더링시 기본 값인 <div> 말고 Row로 렌더링 하도록 해줍니다. */}
                            <Form.Group as={Row} className="mb-3 align-itams-center">
                                <Col xs={3} className="text-center">
                                    <strong>구매 수량</strong>
                                </Col>
                                <Col xs={5}>
                                    {/* 구매 수량은 최소 1이상으로 설정했고, user 모드에 따라서 분기 코딩 했습니다. */}
                                    <Form.Control
                                        type="number"
                                        min="1"
                                        disabled={!user}
