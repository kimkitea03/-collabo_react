...
import { Form, Button, Card, Col, Container, Pagination, Row } from "react-bootstrap";
...
/*
 step01
 상품 목록을 상품 아이디 역순으로 읽어서 화면 전체를 보여줍니다.
 하나의 행에 3개의 열씩 보여줍니다.
...
function App({ user }) {
    ...
    // 페이징과 관련된 satate를 정의합니다.
    const [paging, setPaging] = useState({
        totalElements: 0, // 전체 데이터 개수(69개)
        pageSize: 6, //페이지에 보여 주는 데이터 개수(6개)
        totalPages: 0, // 전체 페이지 개수(12페이지)
        pageNumber: 0, // 현재 페이지 번호(2페이지)
        pageCount: 10, // 페이지 하단 버튼의 개수(10개)
        beginPage: 0, // 페이징 시작 번호 
        endPage: 0, //페이징 끝 번호
        pagingStatus: '', // "pageNumber/ totalPages 페이지"
        searchDateType: '', // 기간 검색 콤보 박스
        category: '', // 검색하고자 하는 특정 카테고리 콤보 박스
        searchMode: '', // 상품 검색 모드 콤보 박스 상품 이름 또는 상품 설명
        ...
        axios
            ...
            .then((response) => {
                ...
                setPaging((previous) => {
                    ...
                    // 만약 pageSize 의 값이 고정적이라면 할당 받지 않아도 됩니다.
    ...
    //이 함수는 관리자 모드일때 보여 주는 '수정'과 '삭제'를 위한 버튼을 생성해주는 함수 입니다.
            ...
            <div className="d-flex justify-content-center">
                ...
                <Button
                    ...
                    onClick={async (event) => {
                        ...
                        if (isDelete === false) {
                            alert(`'${item.name}' 상품을 삭제를 취소하셨습니다.`);
                            return;
                        }

                        try { // 상품을 삭제후 다시 상품 목록 페이지를 보여 줍니다.
                            //주의) 상품을 삭제하려면 반드시 primary key인 상품의 id를 넘겨주어야 합니다.
                            ...
                            alert(`'${item.name}' 상품을 삭제했습니다.`);

                            //삭제된 id를 배제하고, 상품목록 state를 다시 갱신합니다.
                ...
                {products.map((item) => (
                    <Col key={item.id} md={4} className="mb-4">
                        <Card className="h-100"
                            ...
                            />
                            <Card.Body>
                                {/* borderCollapse : 각 셀의 테두리를 합칠 것인지, 별개로 보여줄기를 설정하는 속성 */}
                                <table style={{ width: '100%', borderCollapse: 'collapse', border: 'none' }}>
                                    <tbody>
                                        <tr>
                                            ...
                                            {/* rowSpan 속성은 행방향으로 병합기 가용 ↔ colspan */}
                                            <td rowSpan={2} style={{ width: '70%', padding: '4px', border: 'none', textAlign: 'center', verticalAlign: 'middle' }} >
                                                {makeAdminButtons(item, user, Navigate)}
                                            </td>
                                        </tr>
                                        <tr>
                                            <td rowSpan={2} style={{ width: '70%', padding: '4px', border: 'none' }} >
            ...
            <Pagination className="justify-content-center mt-4">
                ...
                <Pagination.Prev
                    onClick={() => {
                        ...
                        console.log(`prec 버튼 클릭 ${gotopage} 페이지로 이동`)
                ...
                <Pagination.Next
                    onClick={() => {
                        const next = paging.endPage + 1;
                        console.log(`prec 버튼 클릭 ${next} 페이지로 이동`)
                        setPaging((previous) => ({ ...previous, pageNumber: next }));
                    }}
                ...
                <Pagination.Last
                    onClick={() => {
                        const next = paging.totalPages - 1;
                        console.log('first 버튼 클릭')
                        setPaging((previous) => ({ ...previous, pageNumber: next }));
                    }}
                    disabled={paging.pageNumber >= Math.floor(paging.totalPages / paging.pageCount) * paging.pageCount}
